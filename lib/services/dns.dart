...
Future<List<String>> getDnsIpAddresses(String hostname, {String firstChoice = 'cloudflare'}) async {
  ...
  try {
    ...
    final availableProviders = providers.keys.where((p) => p != firstChoice).toList();
    ...
    if (fallbackProviders.isNotEmpty) {
      final fallbackFutures = fallbackProviders
...
bool isIPAddress(String hostname) {
  // IPv4 regex pattern
  final ipv4Pattern =
      RegExp(r'^(\d{1,3}\.){3}\d{1,3}$'); // Matches "192.168.1.1"
  
  // IPv6 regex pattern (simplified)
  final ipv6Pattern = RegExp(r'^([a-fA-F0-9:]+)$'); // Matches "2001:db8::ff00:42:8329"
